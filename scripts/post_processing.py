...
def interpolate_tracks(tracks, interp_factor=None, smooth_factor=20, res=10.0,
                       max_linking_distance=0.05, savgol_polyorder=2,
                       dtype=np.float32, batch_size=1000, gc_frequency=50, use_fast_mode=True):
    """
    Track interpolation with configurable performance options.
    ...
    # drop very short tracks up front
    valid_tracks = []
    for tr in tracks:
        if len(tr) >= 2:
    ...
    for batch_start in tqdm(range(0, len(valid_tracks), batch_size), desc='Interpolating tracks (batch)'):
        ...
        for i, tr in enumerate(batch_tracks):
            ...
            # smooth only if long enough and not fast mode
...
def unnormalize_tracks(tracks, orig_bounds, batch_size=5000, gc_frequency=20, use_vectorized=True):
    """
    Track unnormalization with vectorized operations.
...
def save_tracks_mat(unnorm_tracks, output_matfile, min_length=15):
    ...
    Converts tracks to MATLAB format with columns
    [x, z, vx, vz, timeline] to match MATLAB ULM_tracking2D velocityinterp mode.
    """
    # already filtered at tracking stage
    ...
    for trk in filtered:
        ...
        if N > 1:
            ...
            if dt_min < 1e-8 or np.any(np.diff(t_arr) == 0):
                # degenerate dt, manual differences
                vx = np.zeros(N, dtype=np.float32)
                vz = np.zeros(N, dtype=np.float32)
                if N > 2:
                    dt = t_arr[2:] - t_arr[:-2]
...
def save_tracks_mat_complete(unnorm_tracks, output_matfile, min_length=15, processing_time=None, ulm_params=None):
    ...
    # vertical cell arrays for MATLAB format
